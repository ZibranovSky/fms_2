...
<div class="container-fluid">
	<div class="col-lg-12">
		<div class="row">
			<div class="card col-lg-12" style="background-color:#e1e1e1;">
		...
		<div class="row">
			<div class="col-lg-12" style="">
				<div class="col-md-4 input-group offset-8">
		...
		<div class="row">
			...
			?>
				<div class="card col-md-1 mt-2 ml-2 mr-2 mb-2 folder-item" data-id="<?php echo $row['id'] ?>" style="background-color:#e9e9e9;" >
					<div class="card-body">
						<large><span><img src="assets/img/folder_dua.png" alt="" style="margin-right:20px;"></span><br><b class="to_folder" style="font-size:13px;" > <?php echo $row['name'] ?></b></large>
					</div>
				</div>
			<?php endwhile; ?>
		</div>
		...
		<div class="row">
			<div class="card col-md-12" style="background-color:#f0f0f0;border:none;">
				<div class="card-body">
					<table class="table" width="100%">
						...
						<?php 
							while($row=$files->fetch_assoc()):
								...
								// $arsip_arr =array('rar','zip');
								$pdf_arr =array('pdf','ps','eps','prn');
								$icon ='far fa-file fa-2x';
								if(in_array(strtolower($row['file_type']),$img_arr))
									$icon ='far fa-file-image fa-2x';
								if(in_array(strtolower($row['file_type']),$doc_arr))
									$icon ='far fa-file-word fa-2x';
								if(in_array(strtolower($row['file_type']),$pdf_arr))
									$icon ='far fa-file-pdf fa-2x';
								if(in_array(strtolower($row['file_type']),['xlsx','xls','xlsm','xlsb','xltm','xlt','xla','xlr']))
									$icon ='far fa-file-excel fa-2x';
								if(in_array(strtolower($row['file_type']),['ppt', 'pptm', 'pptx']))
									$icon ='far fa-file-powerpoint fa-2x';
								if(in_array(strtolower($row['file_type']),['zip','rar','tar']))
									$icon ='far fa-file-archive fa-2x';
								if(in_array(strtolower($row['file_type']),['exe']))
									$icon ='fab fa-steam fa-2x';
								if(in_array(strtolower($row['file_type']),['mp3','wav','ogg']))
									$icon ='far fa-file-audio fa-2x';
								if(in_array(strtolower($row['file_type']),['mp4', 'webm', 'ogg', 'mov', 'avi', 'wmv', 'flv', 'swf', 'rm']))
									$icon ='far fa-file-video fa-2x';
								if(in_array(strtolower($row['file_type']),['ppt', 'pptx']))
									$icon ='far fa-file-powerpoint fa-2x';		
								if(in_array(strtolower($row['file_type']),['html', 'php', 'css', 'js']))
									$icon ='far fa-file-code fa-2x';		
								if(in_array(strtolower($row['file_type']),['txt']))
						...
						<tbody>
							<tr class='file-item' data-id="<?php echo $row['id'] ?>" data-name="<?php echo $name ?>">
								<td class="td">
									<large>
										<span class="mr-3"><i class="fa <?php echo $icon ?>"></i></span>
										<b class="to_file"> <?php echo $name ?></b>
									</large>
									<input type="text" class="rename_file" value="<?php echo $row['name'] ?>" data-id="<?php echo $row['id'] ?>" data-type="<?php echo $row['file_type'] ?>" style="display: none">
								</td>
								<td class="td"><p class="to_file"><?php echo date('Y/m/d h:i A',strtotime($row['date_updated'])) ?></p></td>
								<td class="td"><p class="to_file"><?php echo $row['description'] ?></p></td>
								<td class="td"></td>
							</tr>
						</tbody>
					<?php endwhile; ?>
